...
// ──────────────── Hashtag Command Shortcuts Registry ────────────────
const HASHTAG_COMMANDS = [
  {
    id: 'find-food',
    label: '#find-food',
...
function getHashtagCommandResponse(cmdId, thread) {
  ...
  function renderThreadConversation(blockId) {
    ...
    convElem.innerHTML = `
      ...
      <div class="chat-quick-actions">
        ...
        <span class="chat-quick-actions__divider" aria-hidden="true"></span>
      ...
      <!-- Hashtag Command Palette (hidden by default) -->
      ...
      <form class="chat-input-bar" id="form-thread-compose">
        <input 
          ...
          class="chat-input" 
          placeholder="Message ${escapeHtml(thread.title)}... (type # for AI)" 
          autocomplete="off"
          required
        />
    ...
    // ──────── Hashtag Command Palette Logic ────────
    const palette = convElem.querySelector('#hashtag-palette');
    const cmdList = convElem.querySelector('#hashtag-commands-list');
    const composeInput = convElem.querySelector('#input-thread-message');
    let paletteVisible = false;

    function showPalette() {
      if (paletteVisible) return;
      palette.style.display = '';
      requestAnimationFrame(() => palette.classList.add('hashtag-command-palette--visible'));
      paletteVisible = true;
    }
    ...
    function filterCommands(query) {
      ...
      const chips = cmdList.querySelectorAll('.hashtag-cmd-chip');
    ...
    function executeHashtagCommand(cmdId) {
      const cmd = HASHTAG_COMMANDS.find((c) => c.id === cmdId);
      if (!cmd) return;

      // Send the command as a styled outgoing message
      addMessageToThread(blockId, cmd.label);
      composeInput.value = '';
      hidePalette();
      render();

      // After a brief delay, show typing indicator then AI response
      setTimeout(() => {
        ...
        typingBubble.style.background = 'rgba(254, 243, 199, 0.9)';
        typingBubble.style.borderColor = 'rgba(245, 158, 11, 0.5)';
        typingBubble.innerHTML = `
          <span style="font-size: 11px; font-weight: 700; color: #92400E; margin-right: 4px;">WanderBot processing ${escapeHtml(cmd.label)}</span>
        ...
        setTimeout(() => {
          ...
          appendAiResponseCard(resp, cmd);
        }, 900);
      }, 300);
    }

    function appendAiResponseCard(resp, cmd) {
      const feed = container.querySelector('#thread-chat-feed');
      if (!feed) return;

      const card = document.createElement('div');
      card.className = 'chat-message chat-message--incoming chat-message--ai chat-message--hashtag-response';
      card.innerHTML = `
        <div class="user-avatar-initials user-avatar--ai" title="WanderBot AI">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 2a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2 2 2 0 0 1-2-2V4a2 2 0 0 1 2-2z"/>
        ...
        <div class="chat-message__bubble chat-message__bubble--ai">
          ...
          <div class="ai-hashtag-response">
            <div class="ai-hashtag-response__header">
              <span class="ai-hashtag-response__badge" style="color: ${cmd.color}; background: ${cmd.bg};">
                ${cmd.iconSvg}
                <span>${escapeHtml(resp.badge)}</span>
              </span>
              <h4 class="ai-hashtag-response__title">${escapeHtml(resp.title)}</h4>
            </div>
            <p class="ai-hashtag-response__text">${escapeHtml(resp.text)}</p>
            <div class="ai-hashtag-response__items">
              ${resp.items.map((item) => `
                <div class="ai-hashtag-item">
                  <div class="ai-hashtag-item__main">
                    <span class="ai-hashtag-item__name">${escapeHtml(item.name)}</span>
                    <span class="ai-hashtag-item__detail">${escapeHtml(item.detail)}</span>
                  </div>
                  <span class="ai-hashtag-item__tag" style="color: ${cmd.color}; background: ${cmd.bg};">${escapeHtml(item.tag)}</span>
                </div>
              `).join('')}
            </div>
          </div>
          <span class="chat-message__time">Today, ${new Date().toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })} · via ${escapeHtml(cmd.label)}</span>
        </div>
      `;
    ...
    // Input event listener for # detection and filtering
    ...
    const closePaletteBtn = convElem.querySelector('#btn-close-palette');
    if (closePaletteBtn) {
    ...
    cmdList.querySelectorAll('.hashtag-cmd-chip').forEach((chip) => {
      chip.addEventListener('click', () => {
        const cmdId = chip.getAttribute('data-cmd-id');
        executeHashtagCommand(cmdId);
      });
    });

    // Hashtag hint button in quick actions
    const hashtagHintBtn = convElem.querySelector('[data-hashtag-hint]');
    if (hashtagHintBtn) {
      hashtagHintBtn.addEventListener('click', () => {
        composeInput.value = '#';
        composeInput.focus();
        showPalette();
        filterCommands('#');
    ...
    composeForm.addEventListener('submit', (e) => {
      ...
      // Check if text is a hashtag command
      if (text.startsWith('#')) {
        const cmdId = text.replace(/^#/, '').toLowerCase();
        const matched = HASHTAG_COMMANDS.find((c) => c.id === cmdId);
        if (matched) {
          executeHashtagCommand(matched.id);
          return;
        }
      }
